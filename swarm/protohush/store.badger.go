...
func (s *store) EnsureDHPair() (DHPair, error) {
	...
	err = node.Set(dhPairKeypath, nil, dhPair)
	if err != nil {
		return DHPair{}, err
	}
	err = node.Save()
...
func (s *store) getDHPair() (DHPair, error) {
	...
	exists, err := node.Exists(dhPairKeypath)
	if err != nil {
		return DHPair{}, err
	} else if !exists {
		return DHPair{}, types.Err404
	}

	var dhPair DHPair
	err = node.NodeAt(dhPairKeypath, nil).Scan(&dhPair)
	if err != nil {
		return DHPair{}, err
	}
	return dhPair, nil
...
func (s *store) DHPubkeyAttestations() ([]DHPubkeyAttestation, error) {
	...
	err := node.NodeAt(dhPubkeyAttestationsKeypath, nil).Scan(&attestationsByAddr)
	if err != nil {
		return nil, err
	}
...
func (s *store) LatestDHPubkeyFor(addr types.Address) (DHPubkeyAttestation, error) {
	...
	if len(attestations) == 0 {
		return DHPubkeyAttestation{}, types.Err404
	}
...
func (s *store) IncomingIndividualSessionProposals() (map[types.Hash]EncryptedIndividualSessionProposal, error) {
	...
	err := node.NodeAt(incomingIndividualSessionProposalsKeypath, nil).Scan(&proposals)
	if err != nil {
		return nil, err
	}
	return proposals, nil
...
func (s *store) OutgoingIndividualSessionApprovals() (map[types.Address]map[types.Hash]IndividualSessionApproval, error) {
	...
	err := node.NodeAt(outgoingIndividualSessionApprovalsKeypath, nil).Scan(&approvals_)
	if err != nil {
		return nil, err
	}
...
func (s *store) LatestIndividualSessionWithUsers(sessionType string, aliceAddr, bobAddr types.Address) (IndividualSessionProposal, error) {
	...
	session, err := s.IndividualSession(IndividualSessionID{sessionType, aliceAddr, bobAddr, maxEpoch})
	if err != nil {
		return IndividualSessionProposal{}, err
	}
	return session, nil
...
func (s *store) OutgoingIndividualMessageIntents() ([]IndividualMessageIntent, error) {
	...
	err := node.NodeAt(outgoingIndividualMessagesKeypath, nil).Scan(&intentsMap)
	if err != nil {
		return nil, err
	}
...
func (s *store) IncomingIndividualMessages() ([]IndividualMessage, error) {
	...
	err := node.NodeAt(incomingIndividualMessagesKeypath, nil).Scan(&mss)
	if err != nil {
		return nil, err
	}
...
func (s *ratchetSessionStore) loadSharedKey(sessionIDBytes []byte) ([]byte, error) {
	...
	session, err := (*store)(s).IndividualSession(sessionID)
	if err != nil {
		return nil, err
	}
...
var (
	...
	dhPubkeyAttestationsKeypath = rootKeypath.Pushs("dh-pubkey-attestations")
	dhPairKeypath               = rootKeypath.Pushs("dh-pairs")
...
func dhPubkeyAttestationsKeypathFor(addr types.Address) state.Keypath {
	return dhPubkeyAttestationsKeypath.Pushs(addr.Hex())
}
...
func individualSessionsKeypathForUsers(aliceAddr, bobAddr types.Address) state.Keypath {
	...
	return individualSessionsByIDKeypath.Pushs(a.Hex()).Pushs(b.Hex())
}
...
func ratchetPubkeyKeypathFor(pubKey doubleratchet.Key) state.Keypath {
	...
	return messageKeysKeypath.Pushs(hexKey)
}
